...
import { calculateTextPosition, toRadians } from "@/utils/dartgameGeometry";
...
export default function DartSector({
  ...
  const pathData = [
    ...
    `A ${outerRadius} ${outerRadius} 0 ${largeArcFlag} 1 ${points.outer2.x} ${points.outer2.y}`,
    `L ${points.inner2.x} ${points.inner2.y}`,
    `A ${innerRadius} ${innerRadius} 0 ${largeArcFlag} 0 ${points.inner1.x} ${points.inner1.y}`,
    "Z",
  ].join(" ");
  ...
  // Secteur externe avec numéro et interaction
  if (type === "outer") {
